...
PLATE_WEIGHTS_PATH = "mask_rcnn_plate_0008.h5"  # TODO: update this path

config = train.PlateConfig()
#PLATE_DIR = os.path.join(ROOT_DIR, "Dataset")
...
#dataset = train.PlateDataset()
#dataset.load_plate(PLATE_DIR, "val")
...
weights_path = "mask_rcnn_plate_0008.h5"
...
#                                title="Predictions", show_mask=True, show_bbox=True)
...
cv2.imshow("License_Plate_Localised", crop_img)
cv2.imwrite("License_Plate_Localised.jpg",crop_img)
cv2.waitKey(0)
...
a=pytesseract.image_to_string(Image.open('License_Plate_Localised.jpg'))
print("License Number try 1: " + re.sub("[^a-zA-Z0-9]+", '', a))

blnKNNTrainingSuccessful = loadKNNDataAndTrainKNN()
if blnKNNTrainingSuccessful == False:                               # if KNN training was not successful
    print("\nerror: KNN traning was not successful\n")  # show error message
imgPlate = cv2.imread('License_Plate_Localised.jpg')
...
strChars = recognizeCharsInPlate(imgThresh, listOfPossibleCharsInPlate)
print("License Number try 2: " + strChars)
...
try:
	...
	strChars = ''
print("License Number try 3: " + strChars)

#     val = input("press 0 and then press enter if it is a good image:")
#     if (val==0):
#     	goodcount+=1
# accuracy = (100*goodcount)/246
# print(accuracy)